...
void handleEvent(SDL_Event event, vec3 *cameraPosition, mat3 *cameraOrientation, double translationValue, double rotationDegree,
 double orbitAngle, double panOrbitAngle, double tiltOrbitAngle, bool *isWireframe, bool *isRasterised, bool *isRayTraced, bool *isLight, bool *wrapAround);
...
void drawLight(vector<objContent> o, vec3 cameraPosition, mat3 cameraOrientation, float canvasWidth, float canvasHeight, bool wrapAround);
void update(vector<objContent> o, int focalLength, vec3 cameraPosition, mat3 cameraOrientation, float canvasWidth, float canvasHeight,
 bool isWireframe, bool isRasterised, bool isRayTraced, bool isLight, bool wrapAround);
...
//   drawRasterised(cornellBox, focalLength, cameraPosition, cameraOrientation, canvasWidth, canvasHeight, wrapAround);
  ...
  while(true)
  {
    ...
    if(window.pollForInputEvents(&event)) handleEvent(event, &cameraPosition, &cameraOrientation, translationValue, rotationDegree,
     orbitAngle, panOrbitAngle, tiltOrbitAngle, &isWireframe, &isRasterised, &isRayTraced, &isLight, &wrapAround);

    update(cornellBox, focalLength, cameraPosition, cameraOrientation, canvasWidth, canvasHeight, isWireframe, isRasterised, isRayTraced, isLight, wrapAround);
 ...
 bool isWireframe, bool isRasterised, bool isRayTraced, bool isLight, bool wrapAround)
{
  ...
  if (isRayTraced) {
    ...
    drawLight(o,  cameraPosition,  cameraOrientation,  canvasWidth,  canvasHeight,  wrapAround);
  }
}
...
RayTriangleIntersection getClosestIntersection(vector<objContent> o, vec3 cameraPosition, vec3 rayDirection) {
    ...
    for(int i=0; i < (int)o.size(); i++) {
        for(int j=0; j < (int)o[i].faces.size(); j++) {
            ...
            vec3 aoi = lightPos - o[i].faces[j].vertices[0];
            vec3 startPointVector = cameraPosition - o[i].faces[j].vertices[0];
            vec3 n = glm::cross(e0, e1);
            float dot_product = glm::dot(n, aoi);
            float dist = glm::distance(o[i].faces[j].vertices[0], lightPos);
            ...
            && possibleSolution.y + possibleSolution.z <= 1 && possibleSolution.x < closestDistance) {
                ...
                intersection.distanceFromLight = dist;
                intersection.dot = dot_product;
 ...
 void drawLight(vector<objContent> o, vec3 cameraPosition, mat3 cameraOrientation, float canvasWidth, float canvasHeight, bool wrapAround) {
     ...
     for(int x=0; x < WIDTH; x++) {
         for(int y=0; y < HEIGHT; y++) {
             ...
             ray = vec3(xPoint, yPoint, -(WIDTH/2) / tan(M_PI/8)) * (cameraOrientation);
             intersection = getClosestIntersection(o, cameraPosition, ray);
             int red = intersection.intersectedTriangle.colour.red * intersection.dot / ( 4 * (int)M_PI * (int)intersection.distanceFromLight^2);
             int green = intersection.intersectedTriangle.colour.green * intersection.dot / ( 4 * (int)M_PI *  (int)intersection.distanceFromLight^2);
             int blue = intersection.intersectedTriangle.colour.blue * intersection.dot / ( 4 * (int)M_PI * (int)intersection.distanceFromLight^2);
             int colour = packRGB(red, green, blue);
             if(intersection.distanceFromCamera < INFINITY) {
                 window.setPixelColour(x, y, colour);
             }
         }
